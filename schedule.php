...
<body>
    <div class="dashboard-container">
                    ...
                    <div class="profile">
                        ...
                        <div>
                            ...
                            <span class="fa fa-bell priest_noti" style="color:red">
                                <span
                                    class="<?php echo ($total > 0) ? 'blink' : ''; ?>"><?php echo $total + 1; ?></span>
                            ...
                            <span style="color:red"><sup style="color:red;"
                                    class="flashited"><?php echo $total_p; ?></sup></span>
                            <?php } else {
                                    }
                            ...
                            <?php    } else {
                            ?>
                            <img src="picture_data/<?php echo $_SESSION["user"]["picture_data"]; ?>" alt="Profile"
                                id="profile">
                            <?php } ?>
                        </div>
                    </div>
        ...
        try {
            $pdo = new PDO(DSN, DB_USR, DB_PWD);
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $pdo->prepare("SELECT * FROM `schedule_list` WHERE Status = 'Confirm' AND start_datetime >= CURDATE()");
            $stmt->execute();

$(function(){
		...
		function load5(datefrom,perpage,page,dateto){
		$("#loading").html('<img src="img/loading.gif">');
		$.ajax({
			type: 'post',
			url: 'php/summary_read.php',
			data:{
				datefrom : datefrom,
				perpage : perpage,
				page : page,
	...
	load5($("#datefrom").val(),$("#perpage").val(),1,$("#dateto").val())
	
$(document).on('change','#datefrom',function(){
		load5($(this).val(),$("#perpage").val(),1,$("#dateto").val())
 ...
 $(document).on('change','#perpage',function(){
  ...
  $(document).on('change','#dateto',function(){
	 load5($("#datefrom").val(),$("#perpage").val(),1,$(this).val())
 })
 $(document).on('click','#next',function(){
		 var page = ($('ul.pagination li.active').text())
		// alert(parseInt(page) + 1)
		 load5($("#datefrom").val(),$("#perpage").val(),(parseInt(page) + 1))
	...
	//alert('');
		$(".modal-title").html('You can find the event schedule and add request documents here.')
 ...
 //alert($(this).attr('email'))
		 ...
		 $(document).on('click','.approved_docs',function(){
 //alert($(this).attr('email'))
				...
				$.ajax({
					type:'post',
					url: 'php/editable_approved_docs.php',
					data:{
						value: value,
						id: id,
						column: column,

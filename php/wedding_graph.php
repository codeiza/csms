...
try{
        ...
        "SELECT DATE_FORMAT(date_of_payment, '%Y-%m') as month, DATE_FORMAT(date_of_payment, '%Y-%m-%d') as day, SUM(price) as amount_value
         FROM payment WHERE event_type = 'wedding'and date_of_payment >= '".$_REQUEST["DateFrom"]."' AND date_of_payment <= '".$_REQUEST["DateTo"]."'
         GROUP BY month, day"
    );
    $stmt->execute();
		$data = array();

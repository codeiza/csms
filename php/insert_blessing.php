...
try {
	...
	$stmt = $pdo->prepare(
		...
		(Event_type,event_location,start_datetime_event)
		VALUES
		(:Event_type,:event_location,:start_datetime_event)"
	);
	$stmt->bindValue(':Event_type', 'Blessing', PDO::PARAM_STR);
	...
	$stmt1->bindValue(':title', 'Blessing', PDO::PARAM_STR);
	$stmt1->bindValue(':event_type', 'Blessing', PDO::PARAM_STR);
...
$pdo = null;
echo "<script>alert('Blessing form submitted successfully! ')</script>";
